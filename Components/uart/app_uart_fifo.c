...
{
  uint32_t available_count = app_cxt->tx_fifo.buf_size_mask
      - FIFO_LENGTH(app_cxt->tx_fifo) + 1;
  return available_count;
}

static void app_uart_fifo_irq_event_handler(struct app_uart_fifo_ctx *app_cxt,
    app_uart_irq_t irq_event)
{
  ...
  case APP_UART_IRQ_IDLE: {
    ...
    if (head_pos != tail_pos) /* Check change in received data */
    {
      ...
      { /* Current position is over previous one */
        ...
        err_code = app_fifo_write(&app_cxt->rx_fifo,
            &app_cxt->rx_irq.buff[tail_pos], &rtx_size);
      ...
      else
      {
        ...
        {
          rtx_size = head_pos;
          err_code = app_fifo_write(&app_cxt->rx_fifo, &app_cxt->rx_irq.buff[0],
              &rtx_size);
#if (defined APP_UART_FIFO_RX_ECHO) && (APP_UART_FIFO_RX_ECHO == 1)
          for(uint32_t i = 0; i < rtx_size; ++i)
          {
      ...
      {
        if (app_cxt->rx_over != (uint8_t) EVT_APP_UART_RX_OVER)
        {
          /* Overflow in RX FIFO */
    ...
    {
      app_uart_event.evt_type = EVT_APP_UART_RX_NO_DATA;
      app_cxt->fp_event(app_cxt, &app_uart_event);
    } /* if (head_pos != tail_pos) */
  ...
  case APP_UART_IRQ_TX:
    ...
    if (err_code == APP_FIFO_OK)
    {
      ...
      /* If FIFO was full new request to receive one byte was not scheduled.
       * Must be done here.
       */
      if (app_cxt->tx_over == (uint8_t) EVT_APP_UART_TX_OVER)
      {
  ...
  case APP_UART_IRQ_ERROR:
    app_uart_event.evt_type = EVT_APP_UART_COMMUNICATION_ERROR;
    app_cxt->fp_event(app_cxt, &app_uart_event);
    break;
...
uint32_t app_uart_fifo_init(app_uart_fifo_ctx_t *app_cxt,
    app_uart_buffers_t *p_buffers)
{
  ...
  app_fifo_init(&app_cxt->rx_fifo, p_buffers->fifo.rx_buf,
      p_buffers->fifo.rx_buf_size);

  // Configure buffer TX buffer.
  app_fifo_init(&app_cxt->tx_fifo, p_buffers->fifo.tx_buf,
      p_buffers->fifo.tx_buf_size);
...
{
  app_fifo_flush(&app_cxt->tx_fifo);
...
uint32_t app_uart_get(app_uart_fifo_ctx_t *app_cxt, uint8_t *p_byte)
{
  ...
  if ((uint8_t) EVT_APP_UART_RX_OVER == rx_ovf)
  {
    app_uart_evt_t app_uart_event;
#if (defined APP_UART_FIFO_RX_DMA) && (APP_UART_FIFO_RX_DMA == 0)
